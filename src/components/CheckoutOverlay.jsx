...
const CheckoutOverlay = ({ show, onClose ,totalprice}) => {
  ...
  const proceedToNextStep = async () => {
    if (step === 1) {
      ...
      try {
        ...
        const response = await postData('shippingaddress/add_address_for_order', {"userid":userid,"address":JSON.stringify(address)});
        if(response?.status)
        {
          setStep(2); 
        }else{
  ...
  const handleAddressChange = (e) => {
    ...
    if (name === 'pincode') {
      ...
      if (value === '123456') {
        setAddress((prev) => ({ ...prev, city: 'Sample City', state: 'Sample State' }));
      } else {
  ...
  const handleSetLoginIn=async()=>{
    try{
      const userid = JSON.parse(localStorage.getItem("USER"))._id
      const response = await postData('shippingaddress/fetch_shipping_address_by_userid', {"userid":userid});
      if(response?.status)
       {
  ...
  >
    <div className="flex justify-between items-center mb-4 sm:mb-8">
      ...
      <h2 className="text-lg sm:text-xl font-semibold text-center w-full">Checkout</h2>
    </div>
    ...
    <div className="w-full sm:w-[353px] h-full sm:h-[637px] flex flex-col mx-auto mt-6 sm:mt-11">
      {!loggedIn && (
        <div>
          ...
          <button className="w-full h-12 bg-indigo-500 text-white rounded-full mt-6 sm:mt-11" onClick={handleSetLoginIn}>
      ...
      {loggedIn && step === 1 && (
        <div className="flex flex-col justify-start items-start gap-4 sm:gap-6 inline-flex">
          <div className="text-[#5f6980] text-lg sm:text-xl font-semibold font-['Inter']">Shipping Address</div>
          <input
            type="text"
            name="firstName"
          ...
          <div className="p-4 w-full sm:w-[325px] h-16 bg-indigo-500 text-white rounded-full flex items-center justify-center mt-6 sm:mt-8">
            <button onClick={proceedToNextStep} disabled={loaderStatus}>{loaderStatus?"Lodding...":"Proceed to Payment"}</button>
          </div>
        </div>
      )}
      ...
      {step === 2 && loggedIn && (
        <div className="flex flex-col justify-start items-start gap-4 sm:gap-6 inline-flex">
          <div className="text-[#5f6980] text-lg sm:text-xl font-semibold font-['Inter']">Payment Method</div>
          <h3 className="text-lg sm:text-xl font-medium text-gray-800 mb-4 sm:mb-6">Choose Payment Method</h3>
      ...
      {step === 3 && (
        <div className="flex flex-col items-center justify-center h-[464px]">
          ...
          <h3 className="text-lg sm:text-xl font-medium text-gray-800 mb-2">Order Placed Successfully!</h3>
